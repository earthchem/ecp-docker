 ...
 */
	...
	if($material_pkey==""){exit();}

	$myrow=$db->get_row("select * from vmatnormal where material_pkey=$material_pkey limit 1");
	
	$sample_id=$myrow->sample_id;
	...
	*/
	...
	$neighborstring="http://new.earthchemportal.org/ecneighborwrapper.php?material_pkey=$material_pkey";
	...
	$contentstring="http://new.earthchemportal.org/ecneighborwrapper.php?material_pkey=$material_pkey".$urlstring;
	$linkstring="http://new.earthchemportal.org/ecneighborwrapper.php?material_pkey=material_pkey".$urlstring;
...
<body class=" yui-skin-sam" style="margin:10px;background:#FFFFFF;">

<div>

	<div>
	
		<h1>Find Samples Similar to '<?=$sample_id?>'</h1>
		
		<div class="exampleIntro">
			<p>This interface allows you to find samples similar to '<?=$sample_id?>' in the EarthChem Portal database.</p>
		<ul>
			<li>The sliders below are used to define a chemistry query for the EarthChem Portal.</li>
			<li>Initially, all sliders are set to +/-5% of values found for Earthchem sample.</li>
			<li>Those items with a value of 0 will not be checked below, but you can still select them to use in the query.</li>
			<li>You can adjust the values used by using the sliders and text boxes below.</li>
		</ul>
		...
		<div style="padding-top:10px;">
			...
			<div style="border:#CCCCCC thin solid;padding:10px;">
		...
		(function () {
			...
			YAHOO.util.Event.onDOMReady(function () {
				...
				var getcount = function () {
					...
					myurl='ecneighborwrapper.php?material_pkey=<?=$material_pkey?>&'+urlstring;
