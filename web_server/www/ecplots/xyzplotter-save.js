...
function openChild(i) {
	...
	var url='xyz_child.php?i=' + i + '&zpointtype=' + zpointtype + '&zpointsize=' + zpointsize + '&zfillcolor=' + zfillcolor + '&zstrokecolor=' + zstrokecolor + '&timestamp=' + timestamp;
...
function redoColumnSelections_xyz(axis){
	...
	//test: document.write(x_column_name);
	...
	//var master_fieldnames_list = document.form_xyz.master_fieldnames_list.value; // The first time the xyz select boxes were displayed, we saved this value. It takes a lot of time to build, so save it.
	var from = document.form_xyz.from.value;
	var where = document.form_xyz.where.value; //debug: alert(from + where );
	//var plotsql = document.form_xyz.plotsql.value;
	
/* loop thru array: for (i=0;i<scripts.length;i++)
{
document.write(scripts[i] + "<br >");
}*/

	// Repopulate the div containing the input fields for x,y,z and their ratios
	http.open('get', 'xyz-write_form_xyz.php?x_column_name=' + x_column_name + '&y_column_name=' + y_column_name + '&z_column_name=' + z_column_name
	...
	set_default_z_intervals_num(); // Reset the # of z intervals to a default #
...
/*/ No longer needed, since I am submitting form_xyz and form_xyz with a single button? Update some hidden form variables in form_xyz. These ought to be a separate function, really, Eileen.
	...
	//document.form_xyz.plotsql.value=plotsql;	
	*/
}
...
function set_default_z_intervals_num() {
// Set a default minimum value for z_intervals_num
//if (document.form_xyz.z_intervals_num.selectedIndex < 2) {
	document.form_xyz.z_intervals_num.selectedIndex = 2; 
//}
} // end function
