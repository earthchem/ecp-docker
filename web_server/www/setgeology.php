...
$searchquery=$db->get_row("select geology from search_query where pkey = $pkey") or die('could not fetch search_query record in setreference.php');
...
function go()
{
	...
	document.volfield.src='geologypoly.php?gids='+newstring;
	document.volfieldworld.src='geologypoly.php?gids='+newstring+'&world=yes';
...
<H1>Set Geologic Province</H1>

<br>

<div style="border:1px dashed #BBBBBB; width:800px; padding:5px; background:#EEEEEE;">
All geological province definitions come from the Global GIS dataset from American Geological Institute and the USGS.
</div>
<br>
...
#$mygeos=$db->get_results("select gid, name from shapegeology where showsearch=TRUE and showgeology=TRUE order by name");
$mygeos=$db->get_results("select gid, name from shapegeology where showsearch=TRUE order by name");
...
<FORM id='setreference' action='searchupdate.php' method='post'>
	<TABLE class='ku_gridgeo' border=0>
		<TR>
			<TD valign="top"><H4>GEOLOGIC PROVINCE</H4></TD>
		</TR>
		<TR>
			<TD valign="top">
				<SELECT id="myselect" SIZE=11 Name = "geology" onChange="go()">
<?
